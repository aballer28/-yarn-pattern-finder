...
export default async function handler(req, res) {
  ...
  // Block local/private network fetches.
  ...
  try {
    const pageResponse = await fetch(pageUrl.toString(), {
      ...
      headers: {
        "user-agent":
          "Mozilla/5.0 (compatible; GarnSwatchImageResolver/1.0; +https://vercel.com/)",
        "accept":
    ...
    const imageUrl = chooseImage(html, pageResponse.url || pageUrl.toString(), name);
    ...
    const imageResponse = await fetch(imageUrl, {
      redirect: "follow",
      headers: {
        "user-agent": "Mozilla/5.0 (compatible; GarnSwatchImageResolver/1.0)",
        "referer": pageResponse.url || pageUrl.toString()
      }
    });
    ...
    if (!contentType.startsWith("image/")) {
      res.status(404).send("Resolved URL is not an image");
      return;
    }
    ...
    res.status(404).send("Image resolution failed");
  }
}
